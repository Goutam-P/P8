...
mongoose.connect("mongodb://127.0.0.1/ssdi", {
          ...
          if (id === userId) {
            ...
            const visiblePhotos = photos.filter((photo) => {
              return (
                ...
                (!photo.isPrivate &&
                  (photo.sharingList.includes(userId) ||
                    photo.sharingList.length === 0))
              );
            });
  ...
  Photo.findOne(
    ...
    function (err, photo) {
      if (err) {
        ...
        !photo ||
        !photo.comments.find(
          (c) => c._id.equals(commentId) && c.user_id.equals(user_id)
        )
      ) {
...
app.post("/photo/like/:photoId", function (request, response) {
  ...
  Photo.findById(photoId, function (err, photo) {
    if (err) {
      console.error("Error in /photo/like/:photoId", err);
      response.status(500).send(JSON.stringify(err));
      return;
    }

    if (!photo) {
      console.error("Photo not found - /photo/like/:photoId", photoId);
    ...
    if (likedIndex === -1) {
      // If the user hasn't liked the photo, add the like
    ...
    photo.save(function (err) {
      if (err) {
        console.error("Error saving photo likes - /photo/like/:photoId", err);
        response.status(500).send(JSON.stringify(err));
        return;
      }

      // Get the updated count of likes for the photo
      const likeCount = photo.likes.length;

      // Send the response with the updated likes and count
      response.json({ likes: photo.likes, likeCount });
    });
  });
});
