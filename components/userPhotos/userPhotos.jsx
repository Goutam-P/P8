...
class UserPhotos extends React.Component {
  ...
  handleLike = (photoId) => {
    ...
    // Use Axios to send the request to add or remove the photo from favorites
    axios.post(`/photo/like/${photoId}`)
      .then((response) => {
        // Update the state to reflect the changes in the user's favorited photos
        const updatedPhotos = this.state.userPhotosDetails.map(photo => {
        ...
        this.setState({ userPhotosDetails: updatedPhotos });
      })
      .catch((error) => {
  ...
  )  {console.log(this.state.userPhotosDetails)
            ...
            this.state.userPhotosDetails.map((item) => (
              <div key={item._id}>
                ...
                >
                  ...
                  >
          like
                </Button> {console.log(this.state.userPhotosDetails)}
                {item?.likes  && <span>{`${item?.likes.length}`}</span>}
                  <Button
                    photo_id={item._id}
                    variant="contained"
